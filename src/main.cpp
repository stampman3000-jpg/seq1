...
static void ToggleStepChordNote(Step& step, int keyIdx) {
    int midiVal = 60 + keyIdx; // Center C4 is 60
    ...
    // Not found: add to first vacant slot
    if (step.note.empty()) {
        step.note = noteStr;
        step.velocity = 3; // Default triggers velocity
        return;
    }
    for (int i = 0; i < 3; ++i) {
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // --- MODAL STEP PROPERTIES POPUP INTERCEPT ---
                if (stepPopupOpen) {
            ...
            // 1. Grid Focus Navigation (No Shift held)
                        if (!isShiftDown) {
                            // Navigate rows and grids on the left side
                            if (stepPopupFocusX == 0) {
                                if (IsKeyPressed(KEY_UP)) {
                                    if (stepPopupFocusY == 1) {
                                        if (stepPopupCondCol >= 8) {
                                            // Move from bottom row (cycle length) to top row (active loops)
                                            stepPopupCondCol -= 8;
                                        } else {
                                            // Move from top row of CONDIT up to RETRIG
                                            stepPopupFocusY = 0;
                                        }
                                } else if (stepPopupFocusY == 2) {
                                    stepPopupFocusY = 1; // Move up to CONDIT
                                    stepPopupCondCol = 8; // Focus on middle-row item
                                } else {
                                    stepPopupFocusY = 2; // Wrap from Retrig up to Length
                                }
                            }
                                if (IsKeyPressed(KEY_DOWN)) {
                                    if (stepPopupFocusY == 0) {
                                        stepPopupFocusY = 1;
                                        stepPopupCondCol = 0; // Focus first bit of loop mask
                                    } else if (stepPopupFocusY == 1) {
                                        stepPopupFocusY = 2; // Transition down to Length
                                    } else {
                                        stepPopupFocusY = 0; // Wrap from Length down to Retrig
                                    }
                                }

                                // Left/Right arrow navigation
                                if (stepPopupFocusY == 1) { // CONDIT (8 columns)
                                    if (IsKeyPressed(KEY_LEFT)) {
                                        int curRow = stepPopupCondCol / 8;
                                        int curCol = stepPopupCondCol % 8;
                                        curCol--;
                                        if (curCol < 0) {
                                            curCol = 7; // Wrap to rightmost
                                        }
                                    ...
                                    if (IsKeyPressed(KEY_RIGHT)) {
                                        ...
                                        curCol++;
                                        if (curCol > 7) {
                                            stepPopupFocusX = 1; // Move focus to CHORD keyboard
                                        } else {
                                            stepPopupCondCol = curRow * 8 + curCol;
                                        }
                                    }
                                } else { // RETRIG (Y=0) or LENGTH (Y=2)
                                    if (IsKeyPressed(KEY_RIGHT)) {
                                        stepPopupFocusX = 1;
                                    }
                                }
                            }
                // Navigate right column (Piano)
                else if (stepPopupFocusX == 1) {
                    if (IsKeyPressed(KEY_LEFT)) {
                        stepPopupChordKey--;
                        if (stepPopupChordKey < 0) {
                            stepPopupFocusX = 0; // Transition back to left column
                            stepPopupChordKey = 0;
                        }
                    }
                    if (IsKeyPressed(KEY_RIGHT)) {
                        ...
                        if (stepPopupChordKey > 12) {
                            stepPopupChordKey = 12; // Clamp to high C
                        }
                    }
                    if (IsKeyPressed(KEY_UP)) {
                        step.chordType--;
                    ...
                    if (popupEditChange != 0) {
                        step.retrigger = std::clamp(step.retrigger + popupEditChange, 0, 16);
                                    ...
                                    if (shiftArrowPressed || IsKeyPressed(KEY_SPACE) || (encoderTurn != 0 && encoderButton)) {
                                        if (stepPopupCondCol < 8) {
                                            // Top Row: standard on/off bitwise toggle
                                            step.condMask ^= (1 << stepPopupCondCol);
                                        } else {
                                            // Bottom Row: radio button selector (clears other denominator bits first)
                                            step.condMask &= ~0xFF00; // Clear upper byte
                                            step.condMask |= (1 << stepPopupCondCol); // Set selected cycle length
                                        }
                                    }
                                }
                else if (stepPopupFocusY == 2) { // LENGTH (0..16)
                    if (popupEditChange != 0) {
                        step.noteLength = std::clamp(step.noteLength + popupEditChange, 0, 16);
                    }
                }
            }
            else if (stepPopupFocusX == 1) { // CHORD
                if (popupEditChange != 0) {
            ...
            // Render and draw only the step properties frame, completely bypassing standard keyboard checks
            BeginTextureMode(oledScreen);
                ClearBackground(BLACK);
                UIState state = {
                    currentScreen, selectedTrack, cursorTrack, cursorStep,
            ...
            continue; // Modal intercept complete
        }

        // --- SYSTEM MENU INTERCEPT ---
        if (systemMenuOpen) {
            ...
            if (isUpDownHeld) {
                ...
                menuRepeatTimer = 0.0f; // Reset timer on release
            }
        ...
        if (numKeyTriggered >= 0) {
            if (isShiftDown && isCtrlDown) {
                ...
                // Handled in Pattern Switch block
            }
            else {
        ...
        // --- Z KEY INTERCEPT TO TOGGLE POPUP ---
                ...
                if (change != 0) {
                    ...
                    if (trk.engineType == ENGINE_SAMPLER) {
                        ...
                        if (synthGridRow == 1) {
                            ...
                            else if (synthGridCol == 8) { if (isStepLock) EditParam(sp.pitchSweepDepth, trk.pitchSweepDepth, change, 0, 99); else trk.pitchSweepDepth = std::clamp(tracks[selectedTrack].pitchSweepDepth + change, 0, 99); }
        ...
        // Draw modal Step popup centered on top
        if (stepPopupOpen) {
            DrawStepPopup(state);
        }
        ...
        BeginDrawing();
            ...
            // Overlay active LFO popups on top of window canvas
            if (lfoPopupOpen) {
                DrawModulationPopup(state);
            }

            // Overlay active Step popups on top of window canvas
            if (stepPopupOpen) {
                DrawStepPopup(state);
            }
